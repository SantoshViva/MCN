...
public class SubscriberProfileController {
	...
	private static final Logger logger = LogManager.getLogger(SubscriberProfileController.class);
	@Autowired
	Environment env;
	
	@Autowired
	JdbcTemplate jdbcTemplate;
	@RequestMapping(value = "/getSubscriberProfileQuery",method = RequestMethod.GET)
	@ResponseBody
	public String getToneIdInforXML(@RequestParam("aparty") String aparty, @RequestParam("bparty") String bparty,@RequestParam("serviceId") String serviceId, HttpServletRequest req,
		...
		logger.info("getSubscriberProfileQuery|aparty="+aparty+"|bparty="+bparty+"|serviceId="+serviceId);
